...
*  $\mathcal{M}=\left\{1,\ldots,P\right\}\setminus\mathcal{V}$ ：マスクしたパッチ集合、 $\mathbf{m}\in\left\{0,1\right\}^C$ ：波長点レベルのマスク指示子
...
で表す。割当はラベルベクトル $\mathbf{a}=(a_1,\ldots,a_{N_{\rm pix}})^\top\in\left\{1,\ldots,K\right\}^{N_{\rm pix}}$ で表し、 $a_i=k$ が「点 $i$ をクラスタ $k$ に割り当てる」ことを意味する。
