...
<details><summary><b>記号の定義</b></summary>
...
* $\mathbf{W}_{\rm ref}\in\mathbb{R}^{1\times W\times C},\mathbf{D}_{\rm ref}\in\mathbb{R}^{1\times W\times C}$ ：白板参照・暗電流参照
...
* $t^*$ ：大津の二値化で得る閾値
* $\mathbf{B}={b_{h,w}}\in\{0,1\}^{H\times W}$ ：木材マスク（1=木材, 0=背景）
* $N$ ：サンプル数、 $n$ ：サンプル番号
* $\mathcal{D}^{(n)}=\{(h,w)\mid b^{(n)}_{h,w}=1\}$ ：サンプル $n$ の木材画素集合
...
* $\mathbf{t}_{\rm cls}$ ：CLSトークン、 $\mathbf{E}_{\rm pos}$ ：位置埋め込み
* $\mathcal{V}\subset\{1,\ldots,P\}$ ：可視パッチインデックス集合（Encoder入力に残す集合）
* ${\rm Enc}_\theta$ ：Encoder、 $\mathbf{h}_{{\rm cls},i}$ ：EncoderのCLS出力
* $d_z$：潜在次元、 $\mathbf{W}_{\rm proj},\mathbf{b}_{\rm proj}$ ：射影ヘッド
* $\mathbf{z}_i\in\mathbb{S}^{d_z-1}$ ：L2正規化した潜在（球面）
* ${\rm Dec}_\phi$ ：Decoder、 $\hat{\mathbf{x}}_i$ ：再構成スペクトル
* $\mathcal{M}=\{1,\ldots,P\}\setminus\mathcal{V}$：マスクしたパッチ集合、 $\mathbf{m}\in\{0,1\}^C$ ：波長点レベルのマスク指示子
...
この系譜は一次元信号であるスペクトル解析にも有効で、局所的吸収帯から広帯域ベースライン変動まで多スケール構造が重畳するスペクトルに対し、波長帯域をマスクして復元させることで、帯域間相関や連続性、形状差分といった本質特徴の学習が促される。特に装置差・散乱・濃度変動など外乱が混在する実データでは、復元を通じて本質情報とノイズを分離する圧力が働き、従来より頑健な潜在表現が期待できる。
...
可視インデックス集合を $\mathcal{V}\subset\{1,\ldots,P\}$ を用いて可視トークンを
...
Encoder で得た潜在表現 $\mathbf{z}_i$ から、元のスペクトル $\mathbf{x}_i\in\mathbb{R}^C$ を再構成するデコーダ ${\rm Dec}_{\phi}$ を定義する。
...
##### パッチ集合 $\mathcal{M}=\{1,\ldots,P\}\setminus\mathcal{V}$ からマスク支持子 $\mathbf{m}\in\{0,1\}^C$ を作成

パッチ $j\in\{1,2,\ldots,P\}$ が覆う波長点インデックス集合を
...
$\mathbf{X}\in\mathbb{R}^{N_{\rm pix}\times C}$ を **全可視**（マスクなし）で学習済み Encoder に入力して得た特徴量行列を $\mathbf{Z}\in\mathbb{R}^{N_{\rm pix}\times d_z}$ とする。各行ベクトル $\mathbf{z}_i$ は $\ell_2$ 正規化されており $\|\mathbf{z}_i\|_2=1$ を満たすとする。

クラスタ数を $K$ とし、クラスタ中心を行に並べた行列
...
で表す。割当はラベルベクトル $\mathbf{a}=(a_1,\ldots,a_{N_{\rm pix}})^\top\in\{1,\ldots,K\}^{N_{\rm pix}}$ で表し、 $a_i=k$ が「点 $i$ をクラスタ $k$ に割り当てる」ことを意味する。
...
\qquad
\text{s.t. }\|\mathbf{c}_k\|_2=1\ \forall k
```
...
と更新する。以上により、埋め込み $\mathbf{Z}$ の球面幾何に整合したクラスタリングが得られる。
...
| A1 | A2 | A3 |
|----|----|----|
| <img src="./images/A1_cluster_labels_latent_ckm.png"> | <img src="./images/A2_cluster_labels_latent_ckm.png"> | <img src="./images/A3_cluster_labels_latent_ckm.png"> |

#### クリ

| B1 | B2 | B3 |
|----|----|----|
| <img src="./images/B1_cluster_labels_latent_ckm.png"> | <img src="./images/B2_cluster_labels_latent_ckm.png"> | <img src="./images/B3_cluster_labels_latent_ckm.png"> |

#### ヒノキ

| C1 | C2 | C3 |
|----|----|----|
| <img src="./images/C1_cluster_labels_latent_ckm.png"> | <img src="./images/C2_cluster_labels_latent_ckm.png"> | <img src="./images/C3_cluster_labels_latent_ckm.png"> |

#### マツ

| D1 | D2 | D3 |
|----|----|----|
| <img src="./images/D1_cluster_labels_latent_ckm.png"> | <img src="./images/D2_cluster_labels_latent_ckm.png"> | <img src="./images/D3_cluster_labels_latent_ckm.png"> |

#### ヤマザクラ

| E1 | E2 | E3 |
|----|----|----|
| <img src="./images/E1_cluster_labels_latent_ckm.png"> | <img src="./images/E2_cluster_labels_latent_ckm.png"> | <img src="./images/E3_cluster_labels_latent_ckm.png"> |

#### ライムウッド

| V1 | V2 | V3 |
|----|----|----|
| <img src="./images/V1_cluster_labels_latent_ckm.png"> | <img src="./images/V2_cluster_labels_latent_ckm.png"> | <img src="./images/V3_cluster_labels_latent_ckm.png"> |

#### ハードメープル

| W1 | W2 | W3 |
|----|----|----|
| <img src="./images/W1_cluster_labels_latent_ckm.png"> | <img src="./images/W2_cluster_labels_latent_ckm.png"> | <img src="./images/W3_cluster_labels_latent_ckm.png"> |

#### ポプラ

| X1 | X2 | X3 |
|----|----|----|
| <img src="./images/X1_cluster_labels_latent_ckm.png"> | <img src="./images/X2_cluster_labels_latent_ckm.png"> | <img src="./images/X3_cluster_labels_latent_ckm.png"> |

#### スプルース

| Y1 | Y2 | Y3 |
|----|----|----|
| <img src="./images/Y1_cluster_labels_latent_ckm.png"> | <img src="./images/Y2_cluster_labels_latent_ckm.png"> | <img src="./images/Y3_cluster_labels_latent_ckm.png"> |

#### オーク

| Z1 | Z2 | Z3 |
|----|----|----|
| <img src="./images/Z1_cluster_labels_latent_ckm.png"> | <img src="./images/Z2_cluster_labels_latent_ckm.png"> | <img src="./images/Z3_cluster_labels_latent_ckm.png"> |

#### クラスタスペクトル

- 左上：Reflectance, 右上：Reflectance(SNV), 左下：Absorbance, 右下: Absorbance(SNV)
<img src="./images/cluster_spectra.png">

- Absorbanceの二次微分（SG）
<img src="./images/cluster_spectra_2nd_derive.png">